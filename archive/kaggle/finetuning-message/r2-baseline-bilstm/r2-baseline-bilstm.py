...
# BASELINE — BiLSTM-MTL: encoder post-CLS → BiLSTM over the post sequence → masked-mean pool → multi-task
# heads (CORAL ordinal + CE classification, no focal). The original paper's BiLSTM-MTL baseline
# (reported macro-F1 0.4194 within-distribution), reimplemented on OUR gold-holdout split/seed.
if IS_KAGGLE:
    ...
    os.environ.setdefault("R2_SEQ_MODEL", "bilstm")   # BiLSTM over the post-CLS sequence
    os.environ.setdefault("R2_ORDINAL_HEAD", "coral") # CORAL ordinal head (the MTL ordinal arm)
    os.environ.setdefault("R2_LOSS_TYPE", "focal")    # irrelevant (w_focal=0)
    os.environ.setdefault("R2_W_CORAL", "0.5")        # MTL: ordinal (CORAL)
    os.environ.setdefault("R2_W_CE", "0.5")           # MTL: classification (CE)
