...
# TIER-1 — Confident Learning (cleanlab) on the noisy LLM-labeled pool. Same gold-holdout protocol as Run B.
# Modes (R2_CL): "diagnostic" = OOF probs + report flagged-label counts/joint (no retrain; cap-safe ~4h);
#                "drop"       = drop CL-flagged pool rows, retrain CV + gold eval;
#                "downweight" = keep rows, ×R2_CL_DOWNWEIGHT sampler weight on flagged rows, retrain + gold eval.
if IS_KAGGLE:
    os.environ.setdefault("R2_GOLD_HOLDOUT", "1")
    os.environ.setdefault("R2_BALANCE", "1")
    os.environ.setdefault("R2_EPOCHS", "10")
    os.environ.setdefault("R2_CL", "diagnostic")
    os.environ.setdefault("R2_CL_OOF_EPOCHS", "4")   # cheaper OOF pass; cleanlab is robust to imperfect probs
    os.environ.setdefault("R2_CL_DOWNWEIGHT", "0.2")
...
if IS_KAGGLE:
    ...
    subprocess.run([sys.executable, "-m", "pip", "install", "-q",
                    "transformers==4.48.2", "scikit-learn>=1.3", "nltk>=3.8", "cleanlab>=2.6"], check=True)
...
class Config:
    ...
    # confident-learning (cleanlab) label-cleaning: "off" | "diagnostic" | "drop" | "downweight"
    cl: str = os.environ.get("R2_CL", "off")
    cl_oof_epochs: int = int(os.environ.get("R2_CL_OOF_EPOCHS", "4"))
    cl_downweight: float = float(os.environ.get("R2_CL_DOWNWEIGHT", "0.2"))
    # data
...
def train_fold(cfg, tr_seq, tr_lab, va_seq, va_lab, tok, fold, sample_weight=None):
    ...
    # sample_weight (CL down-weight): per-row multiplier aligned to tr_lab; ×cl_downweight on flagged rows.
    if cfg.balance or sample_weight is not None:    # class-balanced minibatches (inverse-freq sampling)
        w = (counts.sum() / np.maximum(counts, 1))[np.asarray(tr_lab)] if cfg.balance else np.ones(len(tr_lab))
...
# ── 6b. Tier-1: Confident Learning (cleanlab) on the noisy LLM-labeled pool ───────
@torch.no_grad()
def predict_proba(model, loader, cfg) -> np.ndarray:
    """Return the (N, n_classes) blended probability matrix — same head-aware blend as evaluate()."""
    model.eval()
    use_coral = cfg.w_coral > 0
    use_cls = (cfg.w_ce > 0) or (cfg.w_focal > 0)
    out = []
    for b in loader:
        ...
        cls_p = F.softmax(cls, dim=-1)
        if use_coral and use_cls:
            p = 0.5 * coral_p + 0.5 * cls_p
        else:
            p = coral_p if use_coral else cls_p
        out.append(p.cpu().numpy())
    return np.concatenate(out, axis=0)


def run_confident_learning(cfg: Config, tok, out: Path):
    """Confident Learning (Northcutt et al., JAIR 2021) on the LLM-labeled pool.
    (1) 5-fold CV at cl_oof_epochs → genuinely out-of-fold probs over the whole pool (CL needs OOF probs).
    (2) cleanlab.find_label_issues → per-class flag counts + confident joint (esp. Behavior = R2's W1).
    (3) diagnostic: stop. drop: retrain CV on cleaned pool. downweight: retrain with ×cl_downweight on flagged.
    Gold (CSSRS-500) is NEVER cleaned → the gold comparison stays honest."""
    ...
    p_seq = [s for s, k in zip(seqs, gold) if not k]; p_lab = labels[~gold]
    if cfg.smoke:
        g_seq, g_lab = g_seq[:8], g_lab[:8]; p_seq, p_lab = p_seq[:20], p_lab[:20]
    p_lab = np.asarray(p_lab)
    print(f">>> CL[{cfg.cl}] | pool={len(p_seq)} {np.bincount(p_lab, minlength=4).tolist()} | "
          f"gold={len(g_seq)} {np.bincount(g_lab, minlength=4).tolist()} ({LABEL_NAMES})", flush=True)

    gold_loader = DataLoader(CSSRSDataset(g_seq, list(g_lab), tok, cfg, train=False), batch_size=cfg.batch_size)

    # ── (1) OOF probs over the pool (cheaper epochs; cleanlab tolerates imperfect probs) ──
    oof = np.zeros((len(p_seq), cfg.n_classes), dtype=np.float64)
    skf = StratifiedKFold(n_splits=cfg.n_folds, shuffle=True, random_state=cfg.seed)
    oof_cfg = Config(**{**cfg.__dict__}); oof_cfg.epochs = 1 if cfg.smoke else cfg.cl_oof_epochs
    for fold, (tr_idx, va_idx) in enumerate(skf.split(p_seq, p_lab)):
        tr_seq = [p_seq[i] for i in tr_idx]; tr_lab = p_lab[tr_idx].tolist()
        va_seq = [p_seq[i] for i in va_idx]; va_lab = p_lab[va_idx].tolist()
        _, state = train_fold(oof_cfg, tr_seq, tr_lab, va_seq, va_lab, tok, fold)
        model = HierarchicalDualHead(oof_cfg, pretrained=False).to(DEVICE)
        model.load_state_dict(state)
        va_loader = DataLoader(CSSRSDataset(va_seq, va_lab, tok, oof_cfg, train=False), batch_size=cfg.batch_size)
        oof[va_idx] = predict_proba(model, va_loader, oof_cfg)
        print(f">>> [OOF fold {fold}] filled {len(va_idx)} pool rows", flush=True)
        if cfg.smoke:
            break

    # ── (2) cleanlab: which pool labels does CL distrust? ──
    issues = find_label_issues(labels=p_lab, pred_probs=oof,
                               return_indices_ranked_by="self_confidence", filter_by="prune_by_class")
    issues = np.asarray(issues)
    cj = compute_confident_joint(labels=p_lab, pred_probs=oof)
    print(">>> CONFIDENT JOINT (rows=noisy label, cols=CL-estimated true):", flush=True)
    print(cj, flush=True)
    print(">>> LABEL-ISSUE COUNTS per class (flagged / total = rate):", flush=True)
    for c in range(cfg.n_classes):
        tot = int((p_lab == c).sum())
        flg = int((p_lab[issues] == c).sum()) if len(issues) else 0
        print(f"    {LABEL_NAMES[c]:10s}: {flg:5d} / {tot:5d} = {(flg/max(tot,1)):.3f}", flush=True)
    print(f">>> total flagged: {len(issues)} / {len(p_lab)} = {len(issues)/max(len(p_lab),1):.3f}", flush=True)
    np.savez(out / "cl_issues.npz", issues=issues, oof=oof, p_lab=p_lab, confident_joint=cj)
    print(f">>> saved {out/'cl_issues.npz'}", flush=True)

    if cfg.cl == "diagnostic":
        print(">>> CL diagnostic-only (no retrain). Baseline gold macro-F1 reference = 0.3849 (Run B).", flush=True)
        return

    # ── (3) retrain on the cleaned pool + eval on untouched gold ──
    flagged = np.zeros(len(p_lab), dtype=bool); flagged[issues] = True
    gold_f1, val_f1, best_g, best_state = [], [], -1.0, None
    skf2 = StratifiedKFold(n_splits=cfg.n_folds, shuffle=True, random_state=cfg.seed)
    for fold, (tr_idx, va_idx) in enumerate(skf2.split(p_seq, p_lab)):
        if cfg.cl == "drop":
            tr_idx = tr_idx[~flagged[tr_idx]]                 # drop CL-flagged rows from this fold's train
            sw = None
        else:                                                # downweight
            sw = np.where(flagged[tr_idx], cfg.cl_downweight, 1.0)
        tr_seq = [p_seq[i] for i in tr_idx]; tr_lab = p_lab[tr_idx].tolist()
        va_seq = [p_seq[i] for i in va_idx]; va_lab = p_lab[va_idx].tolist()
        f1v, state = train_fold(cfg, tr_seq, tr_lab, va_seq, va_lab, tok, fold, sample_weight=sw)
        model = HierarchicalDualHead(cfg, pretrained=False).to(DEVICE)
        model.load_state_dict(state)
        gm = evaluate(model, gold_loader, cfg)
        val_f1.append(f1v); gold_f1.append(gm["macro_f1"])
        print(f">>> [fold {fold}] val(LLM)={f1v:.4f} | GOLD macroF1={gm['macro_f1']:.4f} "
              f"MAE={gm['mae']:.4f} QWK={gm['qwk']:.4f} per-class={gm['per_class_f1']}", flush=True)
        if gm["macro_f1"] > best_g:
            best_g, best_state = gm["macro_f1"], state
        if cfg.smoke:
            break
    print(f">>> CL[{cfg.cl}] GOLD macro-F1: mean={np.mean(gold_f1):.4f} std={np.std(gold_f1):.4f} "
          f"folds={[round(x,4) for x in gold_f1]}  (baseline Run B = 0.3849)", flush=True)
    torch.save({"state_dict": best_state, "model_name": cfg.model_name, "n_classes": cfg.n_classes,
                "use_features": cfg.use_features, "label_names": LABEL_NAMES,
                "gold_macro_f1": float(np.mean(gold_f1)), "cl_mode": cfg.cl}, out / "best_model.pt")
...
def main():
    ...
    print(f">>> device={DEVICE} model={cfg.model_name} smoke={cfg.smoke} "
          f"gold_holdout={cfg.gold_holdout} cl={cfg.cl}", flush=True)
    if cfg.cl != "off":
        run_confident_learning(cfg, AutoTokenizer.from_pretrained(cfg.model_name), out)
        return
    if cfg.gold_holdout:
