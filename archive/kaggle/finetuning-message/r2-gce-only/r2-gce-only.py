...
# ABLATION ARM — GCE-only: CORAL head + GCE (KEEP CORAL). Isolates GCE's contribution vs the
# full CORN+GCE (0.402/0.260) and the dual CORAL+Focal baseline (0.385/0.183). 2x2 cell [CORAL × GCE].
...
if IS_KAGGLE:
    ...
    os.environ.setdefault("R2_ORDINAL_HEAD", "coral") # KEEP CORAL head (isolate GCE)
    os.environ.setdefault("R2_LOSS_TYPE", "gce")      # GCE third term
    os.environ.setdefault("R2_GCE_Q", "0.7")
    os.environ.setdefault("R2_W_CORAL", "0.5")        # ordinal-head (CORAL) weight
