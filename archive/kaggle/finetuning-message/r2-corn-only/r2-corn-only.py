...
# ABLATION ARM — CORN-only: CORN head + Focal (KEEP Focal). Isolates CORN's contribution vs the
# full CORN+GCE (0.402/0.260) and the dual CORAL+Focal baseline (0.385/0.183). 2x2 cell [CORN × Focal].
# Same 5-fold x 10ep gold-holdout+balance, same split/seed → apples-to-apples.
if IS_KAGGLE:
    ...
    os.environ.setdefault("R2_FOLDS", "5")            # ~8.8h GPU
    os.environ.setdefault("R2_ORDINAL_HEAD", "corn")  # CORN head
    os.environ.setdefault("R2_LOSS_TYPE", "focal")    # KEEP Focal (isolate CORN)
    os.environ.setdefault("R2_FOCAL_GAMMA", "2.0")
    os.environ.setdefault("R2_W_CORAL", "0.5")        # ordinal-head (CORN) weight
    os.environ.setdefault("R2_W_CE", "0.3")
    os.environ.setdefault("R2_W_FOCAL", "0.2")        # weight on the Focal term
