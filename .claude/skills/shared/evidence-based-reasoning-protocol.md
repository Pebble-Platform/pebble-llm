...
> Ref: [CLAUDE.md](CLAUDE.md) Evidence-Based Reasoning section
...
4. **Cross-directory validation** — Check all directories in the project before recommending architectural changes.
...
1. **Find ALL implementations** — Grep for exports across `lib/`, `app/`, `components/`
2. **Verify ALL usage sites** — Grep for imports of the target
3. **Check ALL directories** — verify across `lib/`, `app/`, `components/`, `hooks/`
4. **Assess impact** — what breaks if removed?
5. **Declare confidence** — X% based on [evidence list]
...
- [ ] No import references remaining
- [ ] No configuration references (.env files, env vars)
- [ ] No test dependencies
- [ ] Impact checked across all directories
