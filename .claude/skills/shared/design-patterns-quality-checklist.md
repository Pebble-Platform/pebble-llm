...
```
Query Function > Service > Server Action > API Route > Component
     ↑ PREFER                                     AVOID ↓
```

- **Database queries** → Query functions in `lib/db/queries/` (not in API routes)
- **Business logic** → Service functions in `lib/services/` (not in API routes or components)
- **Validation** → Zod schemas (not scattered in handlers)
- **Display logic** → Utility functions or custom hooks (not inline in components)
- **Constants** → `consts/` directory (not scattered in components)
- **Data fetching** → SWR hooks in client components (not useEffect)
