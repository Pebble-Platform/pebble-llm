...
- **Understand the current patterns** — how is the code structured? What conventions and patterns are used?
...
1. **Write analysis to file** — Save investigation results to `plans/reports/{task-name}.analysis.md`
...
- [ ] **Wrote analysis to `plans/reports/` (if non-trivial task)**
...
**API routes:** Grep for `export.*GET|POST|PUT|DELETE` in `app/api/`

**Server actions:** Grep for `'use server'` in `app/`

**Query functions:** Grep for `export.*function` in `lib/db/queries/`

**Components:** Grep for `export.*function|export.*const` in `components/`
...
- **DO NOT** guess at function signatures or API contracts
