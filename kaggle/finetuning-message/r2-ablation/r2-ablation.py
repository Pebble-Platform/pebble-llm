...
# ABLATION — ordinal-head arm: flat-CE (w_coral=0, w_ce=1, w_focal=0) vs the dual-head baseline (Run B).
# Same gold-holdout + balance protocol so the only changed variable is the loss/head composition.
if IS_KAGGLE:
    os.environ.setdefault("R2_GOLD_HOLDOUT", "1")
    os.environ.setdefault("R2_BALANCE", "1")
    os.environ.setdefault("R2_EPOCHS", "10")
    os.environ.setdefault("R2_W_CORAL", "0.0")   # flat-CE arm: CORAL head off
