...
# BASELINE — plain-RoBERTa-CE: encoder → masked-mean pool (NO sequence transformer, NO ordinal head) →
# linear CE. The simplest encoder floor on the SAME gold-holdout split/seed → measures how much the
# seq-transformer + ordinal machinery actually buys over a flat encoder. Reference: PAPER-PLAN §5 baselines.
if IS_KAGGLE:
    os.environ.setdefault("R2_GOLD_HOLDOUT", "1")
    os.environ.setdefault("R2_BALANCE", "1")
    os.environ.setdefault("R2_EPOCHS", "10")          # 5-fold x 10ep (apples-to-apples)
    os.environ.setdefault("R2_FOLDS", "5")
    os.environ.setdefault("R2_SEQ_MODEL", "mean")     # no seq model — masked-mean over posts
    os.environ.setdefault("R2_ORDINAL_HEAD", "coral") # built but untrained (w_coral=0); eval uses CE head only
    os.environ.setdefault("R2_LOSS_TYPE", "focal")    # irrelevant (w_focal=0)
    os.environ.setdefault("R2_W_CORAL", "0.0")        # flat CE only
    os.environ.setdefault("R2_W_CE", "1.0")
    os.environ.setdefault("R2_W_FOCAL", "0.0")
...
class Config:
    ...
    w_coral: float = float(os.environ.get("R2_W_CORAL", "0.5"))   # weight on the ORDINAL head (coral|corn)
    w_ce: float = float(os.environ.get("R2_W_CE", "0.3"))
    w_focal: float = float(os.environ.get("R2_W_FOCAL", "0.2"))   # weight on the 3rd term (focal|gce)
    label_smoothing: float = 0.1
    focal_gamma: float = float(os.environ.get("R2_FOCAL_GAMMA", "2.0"))
    # Direction-D knobs: ordinal head + noise-robust 3rd-term loss
    ordinal_head: str = os.environ.get("R2_ORDINAL_HEAD", "coral")   # coral | corn
    loss_type: str = os.environ.get("R2_LOSS_TYPE", "focal")         # focal | gce
    gce_q: float = float(os.environ.get("R2_GCE_Q", "0.7"))
    # Baseline knob: sequence aggregator over the post-CLS sequence
    seq_model: str = os.environ.get("R2_SEQ_MODEL", "transformer")   # transformer | bilstm | mean
    # training
    ...
    patience: int = 5
    n_folds: int = int(os.environ.get("R2_FOLDS", "5"))
...
class HierarchicalDualHead(nn.Module):
    def __init__(self, cfg: Config, pretrained: bool = True):
        ...
        # sequence aggregator over the post-CLS sequence (transformer default; bilstm/mean = baselines)
        if cfg.seq_model == "transformer":
        ...
        elif cfg.seq_model == "bilstm":
            self.bilstm = nn.LSTM(h, h // 2, num_layers=1, batch_first=True, bidirectional=True)
        ...
        # dual heads — ordinal head is CORAL (shared weight + K-1 thresholds) or CORN (K-1 independent logits)
        if cfg.ordinal_head == "corn":
            self.corn_fc = nn.Linear(h, cfg.n_classes - 1)        # K-1 independent conditional logits
        else:
    ...
    def forward(self, input_ids, attention_mask, valid, dt, feats):
        ...
        if self.cfg.seq_model == "transformer":
        ...
        else:                                                     # baselines: bilstm | mean → masked-mean pool
            s = self.bilstm(e)[0] if self.cfg.seq_model == "bilstm" else e   # (B,S,h)
            m = valid.unsqueeze(-1).float()
            u = (s * m).sum(1) / m.sum(1).clamp_min(1.0)             # masked mean over valid posts
            u = self.pool_drop(u)
        if self.cfg.use_features:
            f = self.feat_mlp(feats)
            u = self.fuse(torch.cat([u, f], dim=-1))
        if self.cfg.ordinal_head == "corn":
            ord_logits = self.corn_fc(u)                         # (B, n_classes-1) conditional logits
        else:
            ord_logits = self.coral_fc(u) + self.coral_bias      # (B, n_classes-1)
...
# ── Direction-D: CORN (Shi et al. 2023) + GCE (Zhang & Sabuncu 2018) ──────────────
def corn_loss(corn_logits, y, n_classes):
    """CORN conditional-probability loss. Task k predicts P(y>k | y>k-1); task k is trained ONLY on the
    conditional subset {i : y_i >= k} (rows still 'in the running'). Unlike CORAL's shared weight, each
    threshold has its own logit, so a noisy rare-class label only contaminates its own task, not all."""
    n_tasks = n_classes - 1
    total, used = corn_logits.new_zeros(()), 0
    for k in range(n_tasks):
        mask = torch.ones_like(y, dtype=torch.bool) if k == 0 else (y > (k - 1))  # y >= k qualifies
        if mask.sum() == 0:
            continue
        lk = corn_logits[mask, k]
        tk = (y[mask] > k).float()
        total = total + F.binary_cross_entropy_with_logits(lk, tk)
        used += 1
    return total / max(used, 1)


def corn_to_probs(corn_logits, n_classes):
    """conditional sigmoids → P(y>k) via chain rule (cumprod) → per-class probabilities."""
    cond = torch.sigmoid(corn_logits)                            # (B, K-1) = P(y>k | y>k-1)
    pgt = torch.cumprod(cond, dim=1)                             # (B, K-1) = P(y>k), monotone ↓
    B = corn_logits.size(0)
    probs = torch.zeros(B, n_classes, device=corn_logits.device)
    probs[:, 0] = 1 - pgt[:, 0]
    for k in range(1, n_classes - 1):
        probs[:, k] = pgt[:, k - 1] - pgt[:, k]
    probs[:, -1] = pgt[:, -1]
...
def gce_loss(cls_logits, y, q):
    """Generalized Cross Entropy (1 - p_y^q)/q. Gradient ∝ p_y^q DOWN-weights low-confidence (likely
    noisy) samples — the opposite of Focal, which up-weights hard/noisy ones."""
    p = F.softmax(cls_logits, dim=-1)
    p_y = p.gather(1, y.unsqueeze(1)).squeeze(1).clamp_min(1e-6)
    return ((1.0 - p_y ** q) / q).mean()


def ord_to_probs(ord_logits, cfg):
    return (corn_to_probs(ord_logits, cfg.n_classes) if cfg.ordinal_head == "corn"
            else coral_to_probs(ord_logits, cfg.n_classes))
...
def evaluate(model, loader, cfg):
    ...
    preds, gts = [], []
    for b in loader:
        ordl, cls = model(b["input_ids"].to(DEVICE), b["attention_mask"].to(DEVICE),
                          b["valid"].to(DEVICE), b["dt"].to(DEVICE), b["feats"].to(DEVICE))
        # only blend heads that were actually trained → valid ordinal ablation (flat-CE / ordinal-only / dual)
        use_coral = cfg.w_coral > 0
        use_cls = (cfg.w_ce > 0) or (cfg.w_focal > 0)
        coral_p = ord_to_probs(ordl, cfg)
        ...
        for step, b in enumerate(tr):
            with torch.autocast(device_type=DEVICE.type, enabled=use_amp):
                ordl, cls = model(b["input_ids"].to(DEVICE), b["attention_mask"].to(DEVICE),
                                  b["valid"].to(DEVICE), b["dt"].to(DEVICE), b["feats"].to(DEVICE))
                y = b["label"].to(DEVICE)
                ord_l = (corn_loss(ordl, y, cfg.n_classes) if cfg.ordinal_head == "corn"
                         else coral_loss(ordl, y, cfg.n_classes))
                third = (gce_loss(cls, y, cfg.gce_q) if cfg.loss_type == "gce"
                         else focal_loss(cls, y, alpha, cfg.focal_gamma))
                loss = (cfg.w_coral * ord_l
