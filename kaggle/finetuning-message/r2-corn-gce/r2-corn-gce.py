...
# DIRECTION D — noise/shift-robust ordinal loss: CORN head (replaces CORAL's shared weight) + GCE
# (replaces Focal) for LLM-weak-label clinical ordinal classification. Tests whether removing CORAL's
# shared-weight cross-contamination + Focal's noise-amplification recovers gold macro-F1 / Behavior-F1.
# CHEAP-VERIFY config: 3-fold x 6-epoch on the gold-holdout protocol (~3 GPU-h), comparable to flat-CE
# (0.4215) and dual-head (0.3849), same split/seed. The R2_W_FOCAL slot now weights the GCE term.
if IS_KAGGLE:
    os.environ.setdefault("R2_GOLD_HOLDOUT", "1")
    os.environ.setdefault("R2_BALANCE", "1")
    os.environ.setdefault("R2_EPOCHS", "10")         # OFFICIAL: 5-fold x 10ep (apples-to-apples vs flat-CE/dual)
    os.environ.setdefault("R2_FOLDS", "5")           # OFFICIAL: 5-fold (~8.8h GPU)
    os.environ.setdefault("R2_ORDINAL_HEAD", "corn") # corn | coral
    os.environ.setdefault("R2_LOSS_TYPE", "gce")     # gce  | focal  (third loss term)
    os.environ.setdefault("R2_GCE_Q", "0.7")
    os.environ.setdefault("R2_W_CORAL", "0.5")       # ordinal-head (CORN) weight
    os.environ.setdefault("R2_W_CE", "0.3")
    os.environ.setdefault("R2_W_FOCAL", "0.2")       # weight on the GCE term
